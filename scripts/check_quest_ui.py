...
with tempfile.TemporaryDirectory() as folder:
    ...
    next(t for t in app.text_input if t.label=='Personal login ID').input('DEMO-264')
    ...
    app.query_params['claim']=card
    next(t for t in app.text_input if t.label=='Personal login ID').input('DEMO-137')
    ...
    click('Change demo login')
    next(t for t in app.text_input if t.label=='Personal login ID').input('SCREEN-01')
    click('Enter demo')
