...
def network_html(network, counts, show_companies=False):
    ...
    for i,o in enumerate(organisations):
        ...
        svg.append(f'<g><title>{name} · {o["id"]} · {escape(o["cluster"])}</title><circle cx="{x}" cy="{y}" r="{17 if o["connector"] else 11}" fill="#009641"/><text x="{x}" y="{y+30}" text-anchor="middle" class="company">{escape(o["name"]) if o["connector"] else ""}</text><text x="{x}" y="{y+16}" text-anchor="middle" class="id">{o["id"]}</text></g>')
    if not show_companies:
        for group,(x,y) in zip(groups,centres):
            members=[i for i,o in enumerate(organisations) if o['cluster']==group]
